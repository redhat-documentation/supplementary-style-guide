...
*Description*: _Application Streams_ are multiple versions of RHEL user-space components that are delivered and updated more frequently than the core operating system packages. Application Streams can be packaged as RPMs packages, modules, or Software Collections. Do not confuse with AppStream, which is the repository through which Application Streams are distributed. Note that the AppStream repository also contains components other than Application Streams.

...
*Description*: _Red Hat Process Automation Manager_ is the Red Hat platform for Business Process Management (BPM). By using Red Hat Process Automation Manager, enterprise business and IT users can document, simulate, manage, automate, and monitor business processes and policies.
...
*Description*: In Red Hat Decision Manager and Red Hat Process Automation Manager, a _rule_ provides the logic that the rule engine executes against. A rule includes a name, attributes, a “when” statement on the left side of the rule that defines the rule conditions, and a “then” statement on the right side of the rule that defines the rule results.
...
*Description*: In Red Hat Decision Manager and Red Hat Process Automation Manager, a user can define a rule structure using a _rule template_. Rule templates provide a placeholder for values and data, and they populate templates to generate many rules.
...
*Description*: In Red Hat Process Automation Manager, the _runtime manager_ interface enables and simplifies the use of a KIE API within processes. The name of the interface is `RuntimeManager`. It provides configurable strategies that control actual runtime execution. The strategies are singleton, per request, and per process instance.

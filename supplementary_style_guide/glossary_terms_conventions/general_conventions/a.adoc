...
*Description*: _Availability_ is the state of being available to perform work in a system, in the context of high availability or availability zones. That is, _availability_ is the ability of a component to be utilized at a given time.

Ensure that _availability_ is not erroneously used to indicate that a server or disk is online or available. For example, do not say "the server has availability".

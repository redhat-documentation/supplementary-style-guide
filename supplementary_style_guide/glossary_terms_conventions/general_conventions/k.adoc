...
*Description*: In Red Hat Process Automation Manager, "KIE" is an abbreviation for "Knowledge Is Everything". _KIE_ is a knowledge solution for Red Hat Process Automation Manager and is used for the generic parts of a unified API, such as building, deploying, and loading.
...
*Description*: In Red Hat Process Automation Manager, the _KIE API_ is a knowledge-centric API, where rules and processes are first class citizens. "KIE" is used for the generic parts of unified API, such as building, deploying, and loading.
...
// BxMS: Added "In Red Hat Process Automation Manager," and reorganized the sentences
[discrete]
[[kjar]]
==== image:images/yes.png[yes] KJAR (noun)
*Description*: In Red Hat Process Automation Manager, _KJARs_ are simple jar files that include a descriptor for the KIE system to produce KieBase and KieSession. Red Hat Process Automation Manager provides a simplified and complete deployment mechanism that is based entirely on Apache Maven artifacts. The KJAR descriptor is represented as the `kmodule.xml` file.
...
*Description*: In Red Hat Process Automation Manager, _knowledge store_ is a centralized repository for your business knowledge. The knowledge store connects to the Git repository to store various knowledge assets and artifacts at a single location.

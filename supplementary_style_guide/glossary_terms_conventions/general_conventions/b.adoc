...
*Description*: "BIOS" is an abbreviation for basic input and output system. The plural form is "BIOSes". BIOS is the specific name for the motherboard firmware in older PCs. Modern computers have a different kind of firmware, called either EFI or UEFI. If you need to refer to computer firmware, do not use BIOS as a general term. Use _firmware_ or a specific phrase like "UEFI firmware or legacy BIOS" instead.

...
*Description*: A _scriptlet_ is a fragment of code in a spec file, usually a shell script, which is executed during build time (build scriptlets) or transaction time (transaction scriptlets) in order to make some additional changes to the user’s system. Build scriptlets are more limited than transaction scriptlets. That is, build scriptlets do not receive arguments and can only be shell scripts.
...
*Description*: _Syndesis_ is the community name for Fuse Ignite.

...
*See also*: xref:bind-n[bind (noun)]
...
*Description*: A _block device_ is a physical storage device that supports reading and writing data in groups (blocks) of bytes in any order. Do not confuse this term with _block storage_ (cloud storage) or _block volume_ (OpenShift Virtualization).
...
*Description*: A _brick_ is the basic unit of storage in Red Hat Gluster Storage. A _brick_ is an exported directory on a server that is in a trusted storage pool.

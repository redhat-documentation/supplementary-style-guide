...
*Description*: In Red Hat Enterprise Linux, _Weak dependencies_ are variants of the Requires directive that describe dependencies that are not necessary for the package to work but are considered useful by the package author. You can think of them as best served with.

...
*Description*: _Application Streams_ are multiple versions of Red Hat Enterprise Linux user-space components that are delivered and updated more frequently than the core operating system packages. Application Streams can be packaged as RPM packages, modules, or Software Collections. Do not confuse with "AppStream", which is the repository through which Application Streams are distributed. Note that the AppStream repository also contains components other than Application Streams.
...
*Description*: The _AppStream repository_  distributes RHEL content as Application Streams and other components. These Application Streams include additional user-space applications, runtime languages, and databases for various workloads and use cases.

...
With this update, the SSG is automatically generated in xref:_markdown_version[Markdown] format in addition to being available as a PDF and in HTML. This change optimizes the SSG for AI consumption and machine reading.

.Glossary entries

* *xref:os[OS]*: Added glossary entry
* *xref:hosted-control-planes[hosted control planes]*: Added glossary entry
* *xref:inference[inference]*: Added glossary entry
* *xref:inferencing[inferencing]*: Added glossary entry
* *xref:inference-serving[inference serving]*: Added glossary entry
* *xref:inferenceservice[InferenceService]*: Added glossary entry

.Style guidance

* *xref:titles-and-headings[Titles and headings]*: Added additional guidance for writing titles and headings for different content types. The guidance includes recommendations for title length and words to avoid. Procedure titles and headings should begin with imperatives instead of gerunds. But do not change any gerund-based headings and titles until after you have converted the content into a Jobs to Be Done format.
* *xref:commands-in-code-blocks[Commands in code blocks]*: Removed the requirement to enclose commands in asterisks for bold formatting, which was breaking commands that contain wildcard characters (`*`).
* *xref:explain-commands-variables-in-code-blocks[Explanation of commands and variables]*: Clarified callout options to provide more flexibility. Description list entries can start with a full sentence, noun phrase, or third-person verb (for example, "Specifies").
* *xref:rh-doc-links[Links to Red Hat documentation]*: Added guidance to use `latest` in links to Red Hat documentation to simplify maintenance. Added guidance for projects that must link to specific versions.
* *xref:release-notes[Release notes]*: Updated examples to align with the rule to avoid "now" for the post-update state.
* *xref:shortdesc[Short descriptions]*: Updated guidance for writing short descriptions that adhere to DITA standards. This guidance includes new diagram examples that show how procedures are structured in both AsciiDoc and DITA and more examples of short descriptions for different content types.
...
* Before opening a pull request (PR) for the SSG, link:https://github.com/redhat-documentation/supplementary-style-guide/issues/[create an issue].
...
See the link:https://github.com/redhat-documentation/supplementary-style-guide/blob/main/CONTRIBUTING.md/[Contributing guide] for more details. 

.Glossary entries

* *xref:key-pair[key pair]*: Added glossary entry
* *xref:remote-execution[remote execution]*: Added glossary entry
* *xref:rex[REX]*: Added glossary entry
* *xref:technology-preview[Technology Preview]*: Updated glossary entry

.Style guidance

* *xref:release-notes[Release note updates]*: To address commonly encountered problems with release notes, the release note improvement initiative made several additions and changes to guidance in the SSG:
** Improved the *xref:style-advice-for-release-note-texts[style advice for release note texts]* 
** Clarified guidance for the correct *xref:tenses-in-release-notes[tenses]* to use in release notes
** Recommended adding *xref:headings-for-release-notes[headings]* to each release note
** Clarified advice on *xref:jira-links-release-notes[references to Jira in release notes]*
** Provided instructions for *xref:release-note-asciidoc-format[release note formatting in AsciiDoc]* , which will simplify DITA migration 
** Provided a single set of approved names for all *xref:release-note-types[release note types and sections]* 
* *xref:commands-in-code-blocks[Commands in code blocks]*. The guidance in this section has changed. When formatting code blocks, put the command input and the example output in separate code blocks. In November, we also updated and improved the ASCIIDoc example for this update. 
* *xref:explain-commands-variables-in-code-blocks[Explanation of commands and variables used in code blocks]*. Customer Content Services is no longer using callouts in documentation. Callouts are not supported by DITA.
...
* *xref:content-view[content view]*: Updated glossary entry to use lowercase spelling
* *xref:nmstate[Nmstate]*: Added glossary entry
* *xref:on-premise[on-premise]*: Updated glossary entry to include Red{nbsp}Hat OpenShift Container Platform definition
* *Product*: Removed glossary entry because Red{nbsp}Hat Satellite no longer capitalizes it
* *Red{nbsp}Hat Network Proxy Server*: Removed glossary entry because this term has been replaced by "Capsule Server"
* *Red{nbsp}Hat Network Satellite Server*: Removed glossary entry because this term has been replaced by "Satellite Server"
* *xref:subscription-manifest[subscription manifest]*: Updated glossary entry to use lowercase spelling

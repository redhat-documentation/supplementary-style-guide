...
*Description*: _Raw-interpreted programs_ are programs that do not need to be compiled and are directly run by a language interpreter.
...
*Description*: _Rolling Streams_, also reffered to as Rolling Application Streams, include Red Hat Enterprise Linux tools and applications that are updated frequently. Later versions of these streams replace earlier versions. Therefore, only one version at a time is supported.
...
*Description*: _RPM_ is the recursive initialism for the _RPM Package Manager_. RPM manages files in the RPM format. Those files are referred to as "RPM packages". Note that even though RPM packages are known informally as `rpm` files, this informal term must not be used in Red Hat documentation to avoid confusion with the command name.

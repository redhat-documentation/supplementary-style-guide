...
*Description*: _Application Streams_ are multiple versions of RHEL user-space components which are delivered and updated more frequently than the core operating system packages. Application Streams can be packaged as RPMs packages, modules, or Software Collections. Do not confuse with AppStream, which is the repository through which Application Streams are distributed. Note that the AppStream repository contains also components other than Application Streams.

*Use it*: yes

*Incorrect forms*: AppStream, application stream
...
*Description*: RHEL content in the _AppStream repository_ includes additional user-space applications, runtime languages, and databases in support of the varied workloads and use cases.

*Use it*: yes

*Incorrect forms*: Appstream, appstream

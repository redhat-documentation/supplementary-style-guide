...
*Description*: A _bytecode_ is a non-human-readable instruction set generated by a compiler. Bytecode is typically either executed by a virtual machine (VM) or recompiled into machine code. For example, Java bytecode is executed on the Java Virtual Machine (JVM).

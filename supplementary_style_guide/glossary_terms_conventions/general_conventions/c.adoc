...
* Generically, a _client_ is an entity, such as a machine, process, service, or application, that uses a service provided by a server. Avoid using _client_ to refer to an application that manages configuration of such clients or to refer to applications, tools, and so on.
...
*Description*: Use _compression_ by itself only after first referring to the specific type of compression, for example, network compression, data compression, migration compression.
...
*Description*: In a data storage context, to _copy_ is the process of creating a duplicate of a file in a different location.

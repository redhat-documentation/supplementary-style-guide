...
==== image:images/yes.png[yes] CodeReady Linux Builder (CRB) repository (noun)
*Description*: The RHEL _CodeReady Linux Builder repository_ provides additional packages for use by developers. Packages included in the CodeReady Linux Builder repository are unsupported.

*Use it*: yes; use it expanded and always mention that packages in this repository are unsupported (even in a release note, for example)

...
[[inference]]
==== image:images/yes.png[yes] inference (noun, verb)
...
*See also*:xref:egress[egress], xref:ingress-controller[Ingress Controller]
...
*Description*: _Internal-mode_ describes using Red{nbsp}Hat OpenShift Container Storage where all components of OpenShift Container Storage are deployed within a cluster managed by OpenShift Container Platform and benefit from Operator-based deployment and management. For example, this situation can occur when you are using internally hosted services.

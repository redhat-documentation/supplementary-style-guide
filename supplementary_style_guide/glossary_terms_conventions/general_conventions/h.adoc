...
*Description*: The _hosted control planes_ model is a control plane configuration where you can manage your workloads, resources, and policies across a hybrid-cloud environment. On first use of the term, use "hosted control planes for OpenShift Container Platform". Capitalize the "H" in "hosted control planes" when you use the term at the start of a title, heading, or sentence. On first use in ROSA documentation, use "For Red{nbsp}Hat OpenShift Service on AWS (ROSA)"; thereafter, use "ROSA with HCP". Do not use "HyperShift" to refer to hosted control planes. 
